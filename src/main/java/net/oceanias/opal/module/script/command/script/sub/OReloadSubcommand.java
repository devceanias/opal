...
public final class OReloadSubcommand extends OSubcommand {
    ...
    public @NotNull @Unmodifiable List<String> getDescription() {
        return List.of("Reloads a script.");
    }

    @Override
    public Argument<String> getSubcommand() {
        final String[] scripts = OScriptManager.get().getLoadedScripts().values().stream()
            ...
            .thenNested(
                new StringArgument("label").replaceSuggestions(ArgumentSuggestions.strings(scripts))
                    .executes((sender, arguments) -> {
                        ...
                        script.reload().thenAccept(result ->
                            OTaskHelper.runTask(() -> {
                                final String message = switch (result) {
                                    case EXECUTION_SUCCESS ->
                                        "&fThe script &6" + script.getLabel() + " &fhas been &ereloaded&f.";
                                    case ALREADY_ENABLED, ALREADY_DISABLED -> null;
                                    case COMPILATION_ERROR, RUNTIME_ERROR, IO_ERROR ->
                                        "&fAn &cerror occurred &fwhile reloading this script! &cCheck console.";
                                };
