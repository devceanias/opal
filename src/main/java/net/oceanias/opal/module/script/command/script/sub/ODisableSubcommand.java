...
public final class ODisableSubcommand extends OSubcommand {
    ...
    public @NotNull @Unmodifiable List<String> getDescription() {
        return List.of("Disables a script.");
    }

    @Override
    public Argument<String> getSubcommand() {
        final String[] scripts = OScriptManager.get().getEnabled().stream()
            ...
            .thenNested(
                new StringArgument("label").replaceSuggestions(ArgumentSuggestions.strings(scripts))
                    .executes((sender, arguments) -> {
                        ...
                        final String message = switch (script.disable()) {
                            case EXECUTION_SUCCESS ->
                                "&fThe script &6" + script.getLabel() + " &fhas been &cdisabled&f.";
                            case ALREADY_ENABLED -> null;
                            case ALREADY_DISABLED -> "&fThis script is &calready disabled&f!";
                            case COMPILATION_ERROR, RUNTIME_ERROR, IO_ERROR ->
                                "&fAn &cerror occurred &fwhile disabling this script! &cCheck console.";
                        };
